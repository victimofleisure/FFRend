...
/*
		...
		03		21may10	adapt GotoUrl for multithreaded apartment
...
{
	// opening URL via direct ShellExecute fails if COM was initialized for a
	// multithreaded apartment; use FileProtocolHandler from url.dll instead
	CString	args;
	args.Format(_T("url.dll,FileProtocolHandler %s"), Url);
	int	retc = (int)ShellExecute(NULL, NULL, 
		_T("rundll32.exe"), args, NULL, SW_SHOWNORMAL);
