...
/*
		...
		07		06mar10	move monitor info to its own class
		08		06mar10	add GetFront and GetBack
		09		10may12	in SetExclusive, add pre-exclusive style and size
...
public:
	...
	IDirectDraw7	*GetDD();
	IDirectDrawSurface7	*GetFront();
	IDirectDrawSurface7	*GetBack();
...
private:
	...
	LONG	m_PreExclStyle;		// window style before exclusive mode
	CSize	m_PreExclSize;		// window client size before exclusive mode
	WINDOWPLACEMENT	m_PreExclPlace;	// window placement before exclusive mode
};
...
inline IDirectDrawSurface7 *CBackBufDD::GetFront()
{
	return(m_Front);
}

inline IDirectDrawSurface7 *CBackBufDD::GetBack()
{
	return(m_Back);
}
